...
func generateAttributesFromReviews(storeName string, reviews []string) ([]string, error) {
	...
	for range 5 {
		...
		for _, chunk := range chunks {
			...
			prompt := "您是全球頂尖的資料探勘專家，正在為名為「" + fmt.Sprintf("%v", storeName) + "」的店家製作精準的分析報告。讓我們一步一步思考來捕捉評論中的各個屬性:\n" +
				"1. 首先，仔細閱讀下列評論，並從中歸納出能夠完整反映評論主旨的各項具體屬性（注意：單一評論可能同時涉及多個屬性）:\n" +
				fmt.Sprintf("%v", chunkedReviewsStr) +
				"2. 屬性的種類包括但不限於：\n" +
...
func analyzeReview(storeName, review string, attributes []string) (*SingleReviewMiningResult, error) {
	...
	prompt := "您是一位全球頂尖的資料探勘專家，正在分析「" + fmt.Sprintf("%v", storeName) + "」（一間商店）的評論。請根據以下評論，找出涉及的屬性，並對每個屬性進行情緒分類。\n" +
		"評論：「" + fmt.Sprintf("%v", review) + "」\n" +
		"- 請根據已識別的屬性，找出評論涉及的類別，並對每個類別進行情緒分類。\n" +
		"- 全部的屬性：`" + fmt.Sprintf("%s", attributes) + "`\n" +
		"- 情緒分類包括：`" + fmt.Sprintf("%s", sentimentClassification) + "`。\n" +
